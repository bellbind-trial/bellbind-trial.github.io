...
現状、テーマのような外部のHugo Modulesは、Hugoプロジェクトごとではなく、マシン共有で別途キャッシュされる。
...
たとえば`hugo server`を立ち上げて、テーマモジュールのセットアップ中などに矯正終了させた場合、その後の`hugo server`でのHTML生成がおかしくなる場合がある。
